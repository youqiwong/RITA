...
def train_model(model, dataset,batch_szie, criterion, optimizer, num_epochs, device, rank, scheduler, num_classes, mvss_protocal, val_config=None, val_batch_size=2):
    best_acc = -torch.inf
    ...
    for epoch in range(num_epochs):
        ...
        results = test_model_distributed(generator=generator, image_size=(image_size, image_size), device=device,num_class=num_classes, is_mvss_protocal=mvss_protocal, dataset_config=val_config, val_batch_size=val_batch_size)
        if rank == 0:
...
def main(image_size):
    ...
    model = train_model(model, dataset,batch_szie, criterion, optimizer, num_epochs, device_local, rank, scheduler,num_classes,mvss_protocal, val_config=val_config, val_batch_size=val_batch_size)
