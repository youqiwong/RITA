...
OUT="${OUT:-${RITA}/robustness_results/sidset_n1000_seed42}"
MANIFEST="${OUT}/manifests/sidset_n1000_seed42.txt"
...
if [ "${#GPU_LIST[@]}" -ne 6 ]; then
  echo "RITA robustness requires exactly 6 GPUs; got: ${GPU_LIST[*]}" >&2
  exit 2
fi

"${PY}" prepare_rita_sidset_robustness.py \
  --output-dir "${OUT}/manifests" --sample-count 1000 --seed 42 --workers 32

PIDS=()
for rank in "${!GPU_LIST[@]}"; do
  CUDA_VISIBLE_DEVICES="${GPU_LIST[$rank]}" "${PY}" infer_rita_sidset_robustness.py \
...
"${PY}" eval_rita_sidset_robustness.py \
  --manifest "${MANIFEST}" --pred-root "${OUT}/pred_masks" \
  --output-dir "${OUT}/tsv" --method RITA-Retrain

echo "Long TSV: ${OUT}/tsv/sidset_robustness_long.tsv"
echo "F1 TSV: ${OUT}/tsv/sidset_robustness_f1.tsv"
echo "IoU TSV: ${OUT}/tsv/sidset_robustness_iou.tsv"
